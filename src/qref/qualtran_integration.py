...
from ._schema_v1 import RoutineV1, _PortV1, _ConnectionV1, _ResourceV1
...
def _extract_common_bloq_attributes(bloq: Bloq, name: Optional[str] = None) -> dict[str, Any]:
    ...
    """
    return {
        "name": bloq.pretty_name() if name is None else name,
...
def _bloq_instance_name(instance: BloqInstance) -> str:
    ...
    which combine a Bloq with a unique ID. When converting such BloqInstance to knifey
...
@singledispatch
def import_from_qualtran(obj, name: Optional[str] = None):
    """Import object from Qualtran by converting it into coresponding knifey object.
    ...
    Return:
        A knifey object corresponding to the source Qualtran object. For both Bloqs
...
@import_from_qualtran.register
def _import_composite_bloq(bloq: CompositeBloq, name: Optional[str] = None) -> RoutineV1:
    ...
    return RoutineV1(
        **_extract_common_bloq_attributes(bloq, name),
        children=[import_from_qualtran(instance) for instance in bloq.bloq_instances],
        connections=[_import_connection(c) for c in bloq.connections],
    )


@import_from_qualtran.register
def _import_bloq(bloq: Bloq, name: Optional[str] = None) -> RoutineV1:
...
@import_from_qualtran.register
def _import_bloq_instance(instance: BloqInstance) -> RoutineV1:
    """Import Bloq (other than CompositeBloq) from Qualtran.
    ...
    """
    return import_from_qualtran(instance.bloq, name=_bloq_instance_name(instance))


def _names_and_dir_from_register(reg: Register) -> Iterable[tuple[str, str]]:
    """Yield names and directions of knifey Ports corresponding to QualtranRegister.
...
def _ports_from_register(reg: Register) -> Iterable[_PortV1]:
    """Given a Qualtran register, return iterable of corresponding knifey Ports.
    ...
    return sorted(
        [
            ...
            # - second one expands composite register (which have no counterpart in Knifey) into
...
def _relative_port_name(soquet: Soquet, direction) -> str:
    """Given a Soquet and direction, determine the relative name of corresponding knifey Port.
    ...
    return (
        # If soquet references BlogqInstance, the corresponding object in knifey
...
def _import_resources(bloq: Bloq) -> list[dict[str, Any]]:
    ...
    for name in resource_names:
        resources.append({"name": name, "value": getattr(t_complexity, name), "type": "additive"})
